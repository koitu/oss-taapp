...
def test_get_messages_success(
    ...
    messages = list(adapter.get_messages(channel_id="channel456", max_results=10))
...
def test_send_message_success(
    mock_send: MagicMock, adapter: ServiceAdapterClient
) -> None:
    """It should send a message to a channel."""
    mock_send.sync.return_value = MessageDetail(
        id="msg_new",
        channel_id="channel456",
    ...
    message = adapter.send_message(channel_id="channel456", content="Hello from test!")

    assert isinstance(message, ServiceMessage)
    assert message.id == "msg_new"
    assert message.content == "Hello from test!"
    assert message.author_name == "TestBot"
...
def test_delete_message_success(
    mock_delete: MagicMock, adapter: ServiceAdapterClient
) -> None:
    """It should delete a message successfully."""
    mock_delete.sync.return_value = OperationResponse(
...
@patch(
    "discord_client_service_adapter.adapter_impl.get_channel_guild_id_channels_channel_id_get"
)
def test_get_channel_success(
    mock_get_channel: MagicMock, adapter: ServiceAdapterClient
) -> None:
    """It should retrieve channel information."""
    mock_get_channel.sync.return_value = ChannelInfo(
        id="channel456", name="general", type_="text"
    )
...
@patch(
    "discord_client_service_adapter.adapter_impl.get_channels_guilds_guild_id_channels_get"
)
def test_get_channels_success(
    mock_get_channels: MagicMock, adapter: ServiceAdapterClient
) -> None:
...
def test_adapter_initialization() -> None:
    ...
    adapter = ServiceAdapterClient(
        service_url="http://example.com:8080", guild_id="my_guild"
    )
